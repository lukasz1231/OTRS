  ...
  <div class="min-h-screen bg-bialeTlo p-8">
    ...
    >
      <div class="bg-white rounded-2xl p-8 w-full max-w-lg shadow-2xl animate-in zoom-in duration-200">
        ...
        <div class="space-y-3 bg-gray-50 p-4 rounded-2xl border border-gray-100">
          <label class="text-xs font-bold text-gray-400 uppercase ml-1">Dodaj agenta do kolejki</label>
          <div class="flex gap-2">
            <select 
              v-model="userIdToAdd" 
              class="flex-grow p-2.5 border border-gray-200 rounded-xl bg-white outline-none focus:ring-2 focus:ring-blue-500 shadow-sm text-sm"
            >
              <option :value="null" disabled>Wybierz pracownika...</option>
              <option v-for="user in allUsers" :key="user.id" :value="user.id">
                {{ user.name }} {{ user.surname }} ({{ user.roles?.join(', ') }})
              </option>
            </select>
            <button
              @click="addUserToQueue"
              class="bg-blue-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-blue-700 transition-all shadow-md active:scale-95"
            >
              DODAJ
            </button>
          </div>
    ...
    >
      <div class="bg-white rounded-3xl p-8 w-full max-w-sm shadow-2xl animate-in zoom-in duration-200 text-center">
        ...
        <div class="flex gap-3">
          <button @click="confirmModal.show = false" class="flex-1 py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-xl transition-colors">
            Anuluj
          </button>
          <button
            @click="confirmModal.action"
            class="flex-1 py-3 bg-red-500 text-white rounded-xl font-bold hover:bg-red-600 shadow-md transition-all active:scale-95"
          >
            Usuń
          </button>
        </div>
...
const fetchQueues = async () => {
  try {
    ...
    console.error('Błąd pobierania kolejek:', e)
  }
}
...
const createQueue = async () => {
  ...
  try {
    ...
    await fetchQueues()
    showNotification('Kolejka została utworzona pomyślnie!', 'success')
  } catch (e) {
    showNotification('Wystąpił błąd podczas tworzenia kolejki.', 'error')
  }
}

const confirmDeleteQueue = (queue) => {
  confirmModal.message = `Zamierzasz usunąć kolejkę "${queue.name}". Tej operacji nie można cofnąć.`
...
const executeDeleteQueue = async (id) => {
  try {
    ...
    await fetchQueues()
    showNotification('Kolejka została usunięta.', 'success')
  } catch (e) {
    confirmModal.show = false
    showNotification('Nie można usunąć kolejki, która zawiera zgłoszenia!', 'error')
  }
}

const confirmRemoveUser = (user) => {
  confirmModal.message = `Czy na pewno chcesz usunąć agenta ${user.name} ${user.surname} z tej kolejki?`
...
const executeRemoveUser = async (uId) => {
  try {
    ...
    await fetchQueues() 
    showNotification('Agent został usunięty z kolejki.', 'success')
  } catch (e) {
    confirmModal.show = false
    showNotification('Błąd podczas usuwania agenta.', 'error')
  }
}
...
const addUserToQueue = async () => {
  ...
  try {
    await axios.post(
      `${API_URL}/queues/${selectedQueue.value.id}/users/${userIdToAdd.value}`,
      {},
      axiosConfig,
    )
    await openManageUsers(selectedQueue.value)
    await fetchQueues()
    showNotification('Agent został dodany do kolejki.', 'success')
  } catch (e) {
    showNotification('Nie udało się dodać agenta.', 'error')
  }
}
