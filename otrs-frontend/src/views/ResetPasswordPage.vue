<template>
  ...
  >
    <div
      class="container max-w-lg flex flex-col bg-white p-8 rounded-2xl shadow-xl transition-all duration-300 ease-in-out"
    >
      <div class="flex flex-col items-center mb-8">
        ...
        <p
          ...
          :class="isSuccess ? 'text-green-600 font-medium' : 'text-tekstSzary'"
        >
          {{
            isSuccess
              ? isTokenMode
      ...
      <form v-else class="flex flex-col" @submit.prevent="handleSubmit">
        <div v-if="isAuthenticatedMode" class="pb-5 flex flex-col space-y-1.5">
          <label for="currentPassword" class="text-sm font-semibold text-tekstSzaryCiemny"
...
const isAuthenticatedMode = computed(() => !isTokenMode.value && userStore.isAuthenticated)
...
onMounted(() => {
  ...
  if (!isTokenMode.value && !isAuthenticatedMode.value) {
    errorMessage.value =
      'Brak uprawnień do zmiany hasła. Zaloguj się lub użyj poprawnego linku resetującego.'
  }
})
