                ...
                <div v-if="isStatusDropdownOpen"
                  ...
                  <div v-for="statusOption in availableStatuses" :key="statusOption.id"
                    @click="handleStatusSelect(statusOption.id)" :class="[
                      ...
                      (isTechnicianOnly && statusOption.id !== 4 && statusOption.id !== 6) ? 'opacity-40 grayscale cursor-not-allowed pointer-events-none' : ''
                    ]">
                    {{ statusOption.name }}
                  </div>
                </div>
...
const handleStatusSelect = async (id) => {
  if (isTechnicianOnly.value && id !== 4 && id !== 6) return;
